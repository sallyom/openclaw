...
export function resolveImplicitAnthropicVertexProvider(params: {
  ...
  return {
    ...
    api: "anthropic-messages",
    // pi-coding-agent's ModelRegistry requires apiKey for custom models.
    // Vertex AI uses GCP credentials (ADC/SA), not API keys — this sentinel
    // satisfies the validation without being used for actual auth.
