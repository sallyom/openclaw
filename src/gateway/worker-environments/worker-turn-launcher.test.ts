...
describe("worker turn launcher local placement", () => {
  ...
  it.each([
    ...
    async ({ nodeDeviceId, providerId, closeFails }) => {
      ...
      const tunnelAuthority = vi.mocked(environments.startTunnel).mock.calls[0]?.[0].authorize;
      expect(tunnelAuthority).toBeTypeOf("function");
      expect(tunnelAuthority).toThrow("Remote-exec turn authority changed during tunnel startup");
