...
describe("gateway worker environment startup", () => {
  it.each(["matching", "session", "environment"] as const)(
    "recovers only the exact interrupted child (%s identity)",
    async (identity) => {
      ...
      await withEnvAsync({ OPENCLAW_STATE_DIR: stateDir }, async () => {
        ...
        if (identity !== "matching") {
          const failed = initial.placementStore.fail({
            sessionId: child.sessionId,
            recoveryError: "replace child",
          });
          if (identity === "session") {
            initial.placementStore.retireSessionPlacement({
              sessionId: child.sessionId,
          ...
          const replacement = {
            ...child,
            sessionId: identity === "session" ? "replacement-session" : child.sessionId,
          };
          const requestedReplacement = initial.placementStore.startDispatch({
            ...replacement,
            executionMode: "worker-turn",
          });
          initial.placementStore.transition({
            ...
            from: "requested",
            to: "provisioning",
            expectedGeneration: requestedReplacement.generation,
            patch: {
              environmentId: identity === "environment" ? "unrelated-environment" : environmentId,
            },
          });
        }
        ...
        try {
          if (identity !== "matching") {
            expect(first.startup.store.get(environmentId)?.destroyRequestedAtMs).toBeNull();
            expect(
              first.startup.store.get("unrelated-environment")?.destroyRequestedAtMs,
            ).toBeNull();
        ...
        try {
          expect(second.startup.placementStore.get(claim.sessionId)).toBeUndefined();
